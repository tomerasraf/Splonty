...
namespace Moonee.MoonSDK.Internal.Analytics.Editor
{
    public class GameAnalyticsPreBuild : IPreprocessBuildWithReport
    {
        ...
        {
            CheckAndUpdateGameAnalyticsSettings(MoonSDKSettings.Load());
        }

        public static void CheckAndUpdateGameAnalyticsSettings(MoonSDKSettings settings)
        {
            if (!settings.GameAnalytics) return;
#if UNITY_ANDROID
